...
For convenience let us introduce the universe `FData` of finitary
data types, the types which are by construction isomorphic to `ℕ` or
to `Fin(n)` for some fixed `n : ℕ`. `FData` includes all quotient
inductive types with closed definitions that do not mention functions
with non-finite domain or other coinductive types. `FData` includes
