...
It's a _construction calculus_, because, besides proofs, it can express constructions: think of algorithms, straightedge and compass constructions in elementary geometry, and universal constructions in category theory. The most basic constructions include numeric datatypes such as (unbounded) integers, (Cauchy) reals, their subsets and spaces of functions between them. More advanced examples include datatypes of expressions for any given formalized language which can be used to carry out proofs by structural induction on term complexity. Following the original Calculus of Constructions, HOCC assumes an impredicative universe of propositions, so it can express structures defined by arbitrary axiomatic theories, including higher order ones (e.g. topologies) and theories with dependent sorts (e.g. categories).
