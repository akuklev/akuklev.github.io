...
- In [“◇Classical and □Parametric Modalities for Martin-Löf Type Theories”](modalities) we propose to add S4 modal operators □ of neccesity and ◇ possibility on types, mapping each proper type `T` to the modal type `□T` of its canonical inhabitants (i.e. representable by finite closed expressions) and the modal type `◇T` of its formal inhabitants (i.e. ones that can be shown to exist using non-constructive reasoning). With these modalities we can enjoy the following without compromising favorable computational properties and decidability of proof/type checking:
