...
Higher Categorical Construction Calculus (HCCC) is a tentative name for a powerful univalent computational type theory we're developing. As a proof calculus, it will be capable of classical reasoning with choice, structural induction over its own language, and higher categorical reasoning. It will enjoy decidable proof checking and will be shown to be a conservative extension of the classical set theory extended with appropriate higher infinity axioms.

We call it a _construction calculus_, because, besides proofs, it can express canonical objects, structures, and algorithmic constructions involving them. Canonical objects include numerical datatypes such as (unbounded) integers, (Cauchy) reals, (pure) functions between and subsets of other expressable datatypes. Besides, we can construct the datatypes of expressions for arbitrary formalized languages and carry out proofs by structural induction on term complexity. Following the original Calculus of Constructions, HOCC assumes an impredicative universe of propositions, so expressable structures include those defined by arbitrary axiomatic theories, possibly higher order and/or with dependent sorts.

We call it _higher categorical_, because instances of these structures, i.a. models of axiomatic theories, come conveniently prepackaged in displayed ω-categories that keep track of structure-respecting correpsondences, homomorphisms, and equivalences on every level, so that all proofs and constructions can be generalized, specialized, and transferred along. Proofs and constructions also transfer outwards: all constructions and proofs about ω-categories and any other theories also apply to their displayed versions, which is crucial since models of an axiomatic theory, including ω-categories themselves, form a displayed higher category, and displayed ω-categories form a displayed displayed ω-category, and so on.
...
- In [Bounded Inductive Construction Calculus](BICC) we are developing a finitistic core theory to use for proving soundness of HOCC: typechecking decidability, canonicity, normalization, conservativity of HCCC over M. Shulman's ZMC/𝕊 and thus relative consistency.
